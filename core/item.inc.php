...
function addItem(){
	$arr=$_POST;
	$path="itemcenterimages";
	$uploadFile=uploadFile($path);
	// $res=insert("item",$arr);
	if($uploadFile&&is_array($uploadFile)){
			$arr['itemPho']=$uploadFile[0]['name'];
		}else{
			return "添加失败";
		}
		if(insert("item", $arr)){
			$mes="添加成功!<br/>正在跳转到项目中心...<meta http-equiv='refresh' content='1;url=itemcenter.php'/>";
		}else{
			$filename=$path.$uploadFile[0]['name'];
			if(file_exists($filename)){
				unlink($filename);
			}
			$mes="添加失败!<br/><a href='additem.php'>重新注册</a>|<a href='index.php'>查看首页</a>";
		}
		return $mes;
}

// 项目点赞
function addItemLikeNo(){
	
}

// 项目评论
function addItemCom(){
	$arr['comCon']=$_POST['comCon'];
	$arr['itemId']=intval($_POST['itemId']);
	$arr['id']=intval($_SESSION['loginFlag']);
	$arr['comDate']=date("y-m-d");
	// $arr['comTime']=time();
	// $arr['comId']=10;
	$mes=insert("comment",$arr);
	// echo $mes;
	// var_dump($arr);
	// exit();
	$mes="评论成功<meta http-equiv='refresh' content='0;url=item.php?itemId=".$arr['itemId']."'/>";
	return $mes;
}
